# GDELT Printed CLI Agent Guide

This directory is a generated `gdelt-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gdelt-pp-cli doctor --json
gdelt-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gdelt-pp-cli which "<capability>" --json
gdelt-pp-cli <command> --help
```
...
```bash
gdelt-pp-cli <command> --help
gdelt-pp-cli <command> --dry-run --agent
```
