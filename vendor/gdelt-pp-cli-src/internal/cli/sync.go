...
Exit codes & warnings:
  ...
  gdelt-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gdelt-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gdelt-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gdelt-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gdelt-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gdelt-pp-cli/data.db)")
