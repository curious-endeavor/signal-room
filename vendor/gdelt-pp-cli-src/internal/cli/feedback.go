...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".gdelt-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GDELT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GDELT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gdelt-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.gdelt-pp-cli/feedback.jsonl.
When ` + "`GDELT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GDELT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gdelt-pp-cli feedback list
  gdelt-pp-cli feedback list --limit 5
  gdelt-pp-cli feedback list --json`,
